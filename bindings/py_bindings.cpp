...
constexpr int kMaxMotionTrackers = 5;
std::array<std::array<double, 7>, kMaxMotionTrackers> MotionTrackerPose;  // Position and rotation for each tracker
std::array<std::array<double, 6>, kMaxMotionTrackers> MotionTrackerVelocity;  // Velocity and angular velocity for each tracker
std::array<std::array<double, 6>, kMaxMotionTrackers> MotionTrackerAcceleration;  // Acceleration and angular acceleration for each tracker
std::array<std::string, kMaxMotionTrackers> MotionTrackerSerialNumbers;  // Serial numbers of the motion trackers
                ...
                if (value.contains("Motion")) {
                    auto& motion = value["Motion"];
                    {
                        ...
                        if (motion.contains("joints") && motion["joints"].is_array()) {
                            ...
                            NumMotionDataAvailable = std::min(static_cast<int>(joints.size()), kMaxMotionTrackers);
...
PYBIND11_MODULE(xrobotoolkit_sdk, m) {
    ...
    m.def("get_motion_tracker_pose", &getMotionTrackerPose, "Get the motion tracker pose data (up to 5 trackers, 7 values each: x,y,z,qx,qy,qz,qw).");
    m.def("get_motion_tracker_velocity", &getMotionTrackerVelocity, "Get the motion tracker velocity data (up to 5 trackers, 6 values each: vx,vy,vz,wx,wy,wz).");
    m.def("get_motion_tracker_acceleration", &getMotionTrackerAcceleration, "Get the motion tracker acceleration data (up to 5 trackers, 6 values each: ax,ay,az,wax,way,waz).");
